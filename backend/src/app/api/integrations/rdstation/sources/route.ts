...
export async function GET() {
  try {
    ...
    const doFetch = async (accessToken: string) =>
      fetch('https://api.rd.services/crm/v2/sources?page[number]=1&page[size]=200', {
        method: 'GET',
        headers: {
    ...
    logger.error('RD Station sources error', { error: error instanceof Error ? error.message : 'Unknown' });
