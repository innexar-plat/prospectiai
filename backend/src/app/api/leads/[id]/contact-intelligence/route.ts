...
export async function GET(
  ...
  try {
    ...
    const data = await buildContactIntelligenceByLeadId(leadId);
    return jsonWithRequestId({ data }, { requestId });
  } catch (error) {
    logger.error('Contact intelligence GET error', {
...
/**
 * PATCH /api/leads/[id]/contact-intelligence
...
export async function PATCH(
  ...
  try {
    ...
    logger.error('Contact intelligence PATCH error', {
      error: error instanceof Error ? error.message : 'Unknown',
    }, requestId);
    return jsonWithRequestId({ error: 'Internal server error' }, { status: 500, requestId });
  }
