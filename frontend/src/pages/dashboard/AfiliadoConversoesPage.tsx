  ...
  return (
    <div className="flex flex-col min-h-0 flex-1">
      ...
      <div className="p-4 md:p-6 max-w-3xl">
        ...
        {!loading && items.length === 0 && <p className="text-muted py-8">{t('page.afiliadoConversoes.empty')}</p>}
        {!loading && items.length > 0 && (
          <div className="space-y-2">
            <p className="text-sm text-muted mb-2">{t('page.afiliadoConversoes.total', { count: total })}</p>
            <ul className="divide-y divide-border rounded-lg border border-border overflow-hidden">
              {items.map((r: Record<string, unknown>) => (
                <li key={String(r.id)} className="px-4 py-3 bg-card text-sm flex flex-wrap gap-x-4 gap-y-1">
                  <span className="text-muted">{r.landedAt ? new Date(String(r.landedAt)).toLocaleDateString() : ''}</span>
                  {r.emailMasked != null && r.emailMasked !== '' && <span>{String(r.emailMasked)}</span>}
                  <span>{r.convertedAt ? t('page.afiliadoConversoes.status.converted') : t('page.afiliadoConversoes.status.signup')}</span>
