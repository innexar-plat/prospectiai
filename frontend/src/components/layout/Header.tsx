    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 px-2 sm:px-3 py-1 pointer-events-none overflow-x-hidden" style={{ paddingTop: 'max(0.25rem, env(safe-area-inset-top))' }}>
            <div className="max-w-7xl mx-auto flex items-center justify-between h-[68px] pointer-events-auto header-wave px-3 md:px-4 rounded-xl shadow-xl border-b border-border/30 min-w-0">
                ...
                {/* Nav links – desktop */}
                <nav className="hidden md:flex items-center gap-1 lg:gap-2 ml-5" aria-label="Navegação principal">
                    <button
                        type="button"
                        onClick={handlePlansClick}
                        className="px-3 py-2 rounded-xl text-sm font-semibold text-muted hover:text-foreground hover:bg-surface transition-colors"
                    >
                ...
                <div className="flex items-center gap-2 md:gap-3 ml-auto">
                    ...
                    <div className="hidden sm:flex items-center gap-0.5 p-1 bg-surface rounded-xl border border-border">
                        <button
                            onClick={() => onLanguageSwitch('pt')}
                            className={`text-[10px] font-bold px-3 py-1.5 rounded-lg transition-all ${locale === 'pt' ? 'bg-violet-600 text-white shadow-lg' : 'text-muted hover:text-foreground hover:bg-surface'}`}
                        >
                            PT
                        </button>
                        <button
                            onClick={() => onLanguageSwitch('en')}
                            className={`text-[10px] font-bold px-3 py-1.5 rounded-lg transition-all ${locale === 'en' ? 'bg-violet-600 text-white shadow-lg' : 'text-muted hover:text-foreground hover:bg-surface'}`}
                        >
                            EN
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        {resultsLength > 0 && (
                            <Button
                                ...
                                size="sm"
                                onClick={() => isPremiumPlan ? onExport() : onPricingRedirect()}
                                icon={isPremiumPlan ? <Download size={14} /> : <span className="text-[10px] grayscale brightness-200">🔒</span>}
            ...
            {/* Mobile Menu: portal so it's not clipped; very high z-index so it stays above hero/landing */}
                ...
                <div className="fixed inset-0 z-[99999] md:hidden" style={{ isolation: 'isolate' }}>
                    ...
                    >
                        <nav className="flex flex-col gap-1" aria-label="Navegação principal">
                            <span className="text-[10px] font-black tracking-widest text-muted uppercase mb-2">Menu</span>
                            <button
                                type="button"
                                onClick={handlePlansClick}
                                className="text-left py-3 px-4 rounded-xl font-semibold text-foreground hover:bg-surface transition-colors"
                            >
                                {locale === 'pt' ? 'Planos' : 'Plans'}
                            </button>
                            <Link
                                to="/privacy"
                                onClick={() => setIsMenuOpen(false)}
                                className="py-3 px-4 rounded-xl font-semibold text-foreground hover:bg-surface transition-colors"
                            >
                                {locale === 'pt' ? 'Privacidade' : 'Privacy'}
                            </Link>
                            <Link
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex gap-2">
                                <button
                                    onClick={() => { onLanguageSwitch('pt'); setIsMenuOpen(false); }}
                                    className={`flex-1 py-3 rounded-xl font-bold text-sm ${locale === 'pt' ? 'bg-violet-600 text-white' : 'bg-surface text-muted'}`}
                                >
                                    Português
                                </button>
                                <button
                                    onClick={() => { onLanguageSwitch('en'); setIsMenuOpen(false); }}
                                    className={`flex-1 py-3 rounded-xl font-bold text-sm ${locale === 'en' ? 'bg-violet-600 text-white' : 'bg-surface text-muted'}`}
                                >
                                    English
                                </button>
                            </div>
