...
export function ProfileCompletenessBanner({
  ...
  const { percent, isCompleteEnough, missing } = useMemo(() => getProfileCompleteness(user), [user]);
      ...
      <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="inline-flex items-center gap-2 rounded-full border border-violet-500/20 bg-violet-500/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-violet-700">
            <Sparkles size={12} /> Perfil estratégico incompleto
          </div>
          <h3 className="mt-3 text-lg font-black text-foreground sm:text-xl">
            Complete o perfil da empresa para gerar análises mais precisas
          </h3>
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted">
            Quanto mais completo estiver o perfil, melhor a IA entende seu contexto comercial, concorrência, ticket, atuação geográfica e proposta de valor.
          </p>
          <p className="mt-2 text-xs text-muted">
            Faltando: {missing.slice(0, 3).join(', ')}{missing.length > 3 ? ' e mais alguns campos.' : '.'}
          </p>
        </div>
